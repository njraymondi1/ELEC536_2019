...
txFlags= SOAPY_SDR_HAS_TIME #| SOAPY_SDR_END_BURST we don't end burst so we can have continuous transmission
...
sdr.activateStream(rxStream, rxFlags, ts)  # take nsamps out of here so we don't specify a buffer size

index = 1
while index < 10:  # psuedo continuous reception
    sr= sdr.readStream(rxStream, [sampsRecv], timeoutUs=int(1e6),numElems=1)
    index = index + 1
print(sr.ret)
if sr.ret!= nsamps:
    print("Bad read!!!")
...
#fftData = np.fft.fft(abs(sig_pad))
